...
* **`MYSPEC`**: one or more of this repository. Each repository contains the substance of the Specification that you're developing.
...
> * Select `Use this template` on the top menu of the GitHub page, and click the green button.
...
If your plan is to have a single repository to develop Technical Specificiations you can now move to [Step 3](#step_3).
...
When creating multiple **MYSpec** repositories, remember to name one of repository as **Governance**. We will customize this repository in [Step 2](#step_2) & [Step 3](#step_3) with only _governance_ documents.
...
**To delete unwanted documents in a repository using GitHub web interface follow these tasks:**
...
As mentioned previously, the Community Specifications contain two different set of documents: _governance_ or _process_, and _development_ or _technical_ documents. 
...
**To update an existing file in a repository follow these steps:**

>  * Go to **`MYSPEC`** repository, (_this name is a placehoder for your specification repository_)
>  * Click on the file_name of the document that you want to modify
> * Click on the`Pencil` icon => *(a new window will open), on edit mode*
>  * Edit the changes listed above
> * Provide a `Title` and `Description` at the bottom of the page
> * Select `Create a new branch` option (*if available*)
> Click on the `Commit changes` green button


From here onwards, TBR
