...
To create a new repository `MYSPEC` in the GitHub Organization's account `MYORG`:
...
To create a new file in a repository follow these steps:

>  * Click `Add file` => `Create new file`
>  * For the filename, enter `FileName.md` (*as indicated below*)
>  * Copy the contents of the default specification template as indicated below, and paste it into the new file's text box. The links provided below point to the raw content of each file to facilicate the copy and paste.
>  * Paste the content into the new file's text box
> * Provie a `Title` and `Description` and commit the content

Copy the following files from the Community Specification 1.1 repository [https://github.com/CommunitySpecification/Community_Specification/tree/V1.1](https://github.com/CommunitySpecification/Community_Specification/tree/V1.1): (*the links below point to the raw content of these files*)
...
> Note: If it is expected that `MYORG` will have multiple repositories containing Technical Specifications, then we suggest that the document [`10._ORG_Process.md`](https://github.com/CommunitySpecification/Community_Specification/blob/V1.1/10._ORG_Process.md) should be copied to a new repository, e.g., `governance`. This document contains processes that are generic across the Organization.
...
To `edit` an existing file, takes these steps:
...
In `MYSPEC` repository fill in the required information for the following files: 
...
If any source or sample code will be included in the specification, designate a source code license in the `4._License.md` file.
  * Under _Specification License_, change _repository_ to _GitHub organization_.
  * Under _Source Code License_, the default license for software source code is **MIT**. If a license other than **MIT** will be used for software source code, then change each instance of **MIT** to the [SPDX License Identifier](https://spdx.org/licenses) for the chosen software license.
...
* `6._Contributing.md`:
  ...
  * If you revise the contents of the policy, also replace _Community Specification_ with `MYORG` in the name of the policy.

### Step 3: Create a new repository `governance` repository
...
1.  **Specification format.**  Use the [Community Specification Template](https://github.com/CommunitySpecification/Community_Specification/blob/V1.1/7._CS_Template.md) to draft your specification.
