...
>If the Organization has a single repository, then the `governance` repository is  not needed, and the technical documents and the ones listed above will be stored on the same repository, **`MYSPEC`**. 
...
> The diagram below depictures the process for copying one or multiple Community Specification Template into the GitHub Organization account.
